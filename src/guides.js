...
function chineseGuide(identity) {
  if (identity === "admin") {
    return [
      ...
      "也可以只输入版本尾码，例如：9110 TGY ZF5；机器人会先补全官方版本号并请求确认。",
      "查询结果可刷新、清缓存或加入监控。",
      "",
      "管理入口：监控、发布链、用户、管理员。",
...
function englishGuide(identity) {
  if (identity === "admin") {
    return [
      ...
      "You can also enter a short revision suffix, for example: 9110 TGY ZF5. The bot shows the full official version and asks for confirmation first.",
...
export function adminHelpParts(lang = "zh") {
  if (lang === "en") {
    return [
      [
        ...
        "/testscan [MODEL CSC] - Scan Samsung version.test.xml once (VPS only; admin only)",
        "/testconfirm - Confirm SM-S948N/KOO, then enable SM-S948B/EUX decryption",
  ...
  return [
    [
      ...
      "/testscan [MODEL CSC] - \u626b\u63cf Samsung version.test.xml\uff08\u4ec5 VPS \u7ba1\u7406\u5458\uff09",
      "/testconfirm - \u786e\u8ba4 SM-S948N/KOO \u540e\u5f00\u542f SM-S948B/EUX \u89e3\u5bc6",
